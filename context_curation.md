...
| Parallelizability | Do subtasks explore independent territory, and would the combined result still hold together if no one coordinated the pieces? | Genuinely independent (no shared style/architecture/schema) → subagents, condensed return. Shares an implicit constraint → still parallelizable, but see the coordination requirement below. |
...
**Check reversibility before committing to a looping pattern.** Evaluator-optimizer and voting both carry a hidden assumption: that repeating the attempt is cheap and safe — generate, check, retry. Whether that holds depends entirely on the Reversibility axis above, not a third category of its own: generating or checking something disposable (a draft, a dry run) is reversible *by construction* — discarding it is the revert primitive — so the pattern applies as described. Re-attempting something with a real, one-way side effect is not reversible — you can't retry a phase that already moved money the way you retry a draft that missed the bar — so move the loop boundary earlier: iterate on a check *before* the irreversible step, never through it.

**Coordination requirement for parallelized work that shares one output.** Independent territory (different files, different sources) isn't the same as independent decisions. Subtasks headed for one combined artifact — a codebase, a design, a document — can carry implicit choices (style, architecture, naming, approach) that conflict even when the pieces themselves never overlap; two workers can each do their job correctly and still produce something incoherent together. This is invisible to a correctness check (§8's "spot-check subagent returns" catches a wrong *finding*, not a mismatched *choice*), so it needs its own guard: before dispatching parallel workers on anything beyond pure fact-finding, hand every worker the same shared spec — whatever the coherence actually depends on — up front, and run an explicit reconciliation pass on the combined result before calling it done. Don't rely on condensed returns alone to surface a conflict neither worker knew was worth mentioning ([Don't Build Multi-Agents](https://cognition.ai/blog/dont-build-multi-agents) — Cognition AI's counter-example to the subagent-delegation guidance above is exactly this failure mode: two independently "correct" workers producing an incoherent combined result).
...
-   **Reversibility** is a separate question from stakes — a cheap action can still be irreversible, and an expensive one can still be trivially revertible. It sets whether §7's fallback mechanism can lean on a revert primitive (checkpoint/undo) or must instead be a compensating action agreed *before* the work starts. Same discovery rule as stakes: action types known at intake get assessed then; a genuinely new action type that surfaces mid-task gets its reversibility checked on the spot *before* it's taken, not after — the point is catching a one-way door before walking through it, not during the postmortem.
...
When a genuinely new action type surfaces mid-task, its stakes-driven guardrail tier (§4) and its reversibility (§7) get triaged together, in the same on-the-spot pass — they're two questions about the one newly discovered action, not two separate procedures run independently of each other.

<details><summary>Why not always run a dedicated orchestrator + user-facing agent?</summary>

It's tempting to default to a two-tier design — a lightweight user-facing agent plus a persistent orchestrator that survives across days. It genuinely earns its complexity when the task requires **true unattended execution** (attention truly decoupled from the conversation). But most "multi-day" needs are really "don't forget where we were," which a *single* reconstructable agent identity solves via persisted external state (progress file, git log) — the pattern Anthropic's long-running-harness work uses ([Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)): same role, same instructions, same tools, reloaded from disk each session. (That source's own example is a coding-specific harness built for full-stack web development — it doesn't make this a general claim itself. What we're generalizing here is the underlying mechanism, persisted state plus a reconstructable identity, not a domain-agnostic recommendation the source actually offers.)
...
**A fourth, complementary technique: compaction.** A progress file solves *what to remember*; compaction solves *what to stop carrying*. As a long single-threaded conversation approaches the context window's limit, summarizing and discarding stale history — keeping architectural decisions and open threads, dropping exhausted tool output — lets the task continue without a subagent split or a full context reset. This is Anthropic's own default recommendation for long single-threaded work in the same source cited above, and it's also what [Don't Build Multi-Agents](https://cognition.ai/blog/dont-build-multi-agents) recommends in place of parallel decomposition for tasks that don't actually need to be split. Use it alongside a progress file, not instead of one — compaction manages the conversation's size, the progress file manages what must survive regardless of size.
...
Pick the tier per action type, scaled to that action's blast radius — not one global policy for the whole task. **Direction matters as much as magnitude, but direction alone doesn't set the tier — check the exit action's own risk first.** An action that increases exposure (advance to the next phase, expand scope) earns its tier from blast radius alone. An action that reduces exposure or reverses a prior one (roll back, disable a flag, abort) is a *candidate* for a lower tier than its raw blast radius suggests — the moment a rollback is actually needed is usually the moment something's already going wrong, and that's exactly when friction costs the most. But being labeled "the rollback" isn't the same as being safe to execute: check that action's own stakes and reversibility (§0) before granting it the discount, the same way §2 checks fragility before inheriting the stakes default rather than assuming it. A rollback that itself carries real risk — failing back to a region whose replica hasn't caught up, running a "down" migration against data the forward migration never accounted for — doesn't get the discount just because its direction is "exit." Categories known at task start get theirs during §0 intake, together with the user. A genuinely new category of risky action that surfaces mid-task (e.g. a bulk-user-reset capability discovered in week 3 of a migration) doesn't require reopening that conversation — the agent applies the same stakes/fragility criteria on the spot and assigns it a tier itself, disclosing that it's doing so, in keeping with this section's disclosure principle below.
...
-   **Threshold** — how readily the agent escalates at all. Driven by stakes: high-stakes work escalates readily, low-stakes exploratory work escalates rarely. One exception: an abort trigger (§6) isn't subject to this dial — a live signal that continuing is actively harmful escalates regardless of how low the task's general threshold is set. Threshold governs ordinary judgment calls, not safety-critical ones.
...
-   A live signal indicates continuing is actively harmful, not just incomplete (§6's abort-trigger case) — stop immediately rather than iterating further, and prefer the lower-friction rollback path (§4's directionality guidance, §7's fallback) over pushing forward. If no safe rollback path exists — the failure mode wasn't anticipated, §7 has no compensating action on file for it — that absence is itself part of what gets escalated: stop and hand the choice to the user rather than picking between two bad options alone.

...
Pick the tier per action type, scaled to that action's blast radius — not one global policy for the whole task. **Direction matters as much as magnitude:** an action that increases exposure (advance to the next phase, expand scope) earns its tier from blast radius alone, but an action that reduces exposure or reverses a prior one (roll back, disable a flag, abort) should default to a *lower* tier than its raw blast radius would suggest — gating the exit as tightly as the entrance defeats the guardrail's own purpose, and the moment a rollback is actually needed is usually the moment something's already going wrong, when speed matters most. Categories known at task start get theirs during §0 intake, together with the user. A genuinely new category of risky action that surfaces mid-task (e.g. a bulk-user-reset capability discovered in week 3 of a migration) doesn't require reopening that conversation — the agent applies the same stakes/fragility criteria on the spot and assigns it a tier itself, disclosing that it's doing so, in keeping with this section's disclosure principle below.
